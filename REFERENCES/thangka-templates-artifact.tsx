...
const THANGKA_TEMPLATES = [
  {
    ...
    renderGuides: (size) => {
      ...
      const paths = [];
      // 1. 最外圍火焰圈
      paths.push(`<circle cx="${center}" cy="${center}" r="${size * 0.46}" stroke="currentColor" stroke-dasharray="8,6" fill="none" opacity="0.35" />`);
      paths.push(`<circle cx="${center}" cy="${center}" r="${size * 0.44}" stroke="currentColor" fill="none" opacity="0.4" />`);
      // 2. 金剛橛/蓮花瓣外圈
      ...
      paths.push(`<circle cx="${center}" cy="${center}" r="${size * 0.35}" stroke="currentColor" fill="none" opacity="0.5" />`);
      // 3. 四正城門 (方殿)
      ...
      for (let i = 0; i < 8; i++) {
        ...
        paths.push(`<path d="M ${x1} ${y1} Q ${cp1x} ${cp1y} ${x2} ${y2} Q ${cp2x} ${cp2y} ${x1} ${y1}" stroke="currentColor" fill="none" opacity="0.6" />`);
      }
      // 5. 中心智慧點
      ...
      for (let i = 0; i < 12; i++) {
        ...
        paths.push(`<line x1="${center}" y1="${center}" x2="${targetX}" y2="${targetY}" stroke="currentColor" stroke-dasharray="3,9" opacity="0.25" />`);
      }
      return paths.join('');
    }
  },
  ...
  },
  {
    ...
    renderGuides: (size) => {
      ...
      paths.push(`<circle cx="${center}" cy="${center}" r="${size * 0.42}" stroke="currentColor" opacity="0.15" stroke-dasharray="10,20" />`);
  ...
  },
  {
    ...
    renderGuides: (size) => {
      ...
      const nodes = [
        [0, -1.6], [1.6, 0.4], [-1.6, 0.4], [0, 2.0],
        [1.6, -1.6], [-1.6, -1.6], [3.2, -0.6], [-3.2, -0.6]
      ];
      nodes.forEach(([nx, ny]) => {
        paths.push(`<circle cx="${center + nx*s}" cy="${center + ny*s}" r="${s*0.25}" stroke="currentColor" stroke-dasharray="1,1" fill="none" opacity="0.4" />`);
      });
...
const ZENTANGLE_PATTERNS = [
  ...
  { name: '烈日火焰 (Paradox)', symbol: '🌀', desc: '純直線拉出的扭轉幾何幻覺', difficulty: '⭐⭐⭐⭐' },
...
export default function App() {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${isDarkMode ? 'bg-[#0A0B10] text-gray-100' : 'bg-gray-50 text-gray-800'}`}>
      
      {/* 頂部裝飾大條 & 標題 */}
      <header className={`border-b ${isDarkMode ? 'border-gray-800 bg-[#0E1017]' : 'border-gray-200 bg-white'} px-6 py-4 sticky top-0 z-50 transition-all`}>
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">融合神聖唐卡圖騰與禪繞畫(Zentangle)的交互冥想空間</p>
            </div>
          </div>
        ...
        <main className="max-w-7xl mx-auto px-6 py-10 animate-fadeIn">
          ...
          <div className={`p-8 rounded-3xl mb-12 relative overflow-hidden border ${
            ...
            <div className="max-w-3xl relative z-10">
              <span className="text-xs font-bold tracking-widest text-amber-500 uppercase">Tibetan Zentangle Art</span>
            ...
            {THANGKA_TEMPLATES.map((tpl) => {
              ...
              return (
                ...
                >
                  <div>
                    {/* SVG 預覽圖 */}
                    <div className={`aspect-square rounded-xl p-4 flex items-center justify-center mb-4 transition-all ${
          ...
          <section className={`mt-16 p-8 rounded-2xl border ${
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-6 text-xs md:text-sm">
              <div className="space-y-1.5">
                ...
                <p className="opacity-75 text-xs leading-relaxed">坐在舒適椅子上，深呼吸三次。開啟「背景頌缽音」，讓外界嘈雜漸漸退去。</p>
              </div>
              <div className="space-y-1.5">
                <div className="font-bold text-amber-500">2. 觀察與對齊</div>
                <p className="opacity-75 text-xs leading-relaxed">看著底稿的神聖幾何，這不是束縛，而是協助您在虛空中建立宇宙秩序的階梯。</p>
              </div>
              <div className="space-y-1.5">
                <div className="font-bold text-amber-500">3. 專注畫線</div>
                <p className="opacity-75 text-xs leading-relaxed">開啟對稱模式。順著底稿格線，重複地畫出一條條波浪、圓圈或網格。不急躁，一筆一畫。</p>
              </div>
              <div className="space-y-1.5">
                <div className="font-bold text-amber-500">4. 填補陰影</div>
                <p className="opacity-75 text-xs leading-relaxed">在線條交錯的地方，使用「玄鐵炭黑」或降低透明度畫筆，描出陰影，營造立體交織感。</p>
              </div>
              <div className="space-y-1.5">
                <div className="font-bold text-amber-500">5. 迴向與欣賞</div>
                <p className="opacity-75 text-xs leading-relaxed">完成後，關閉引導線，看看只屬於你內心秩序的輝煌成果，感恩這段與自己獨處的時光。</p>
              </div>
            </div>
          </section>
        </main>
        ...
        <main className="max-w-7xl mx-auto px-4 md:px-6 py-6 animate-fadeIn">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              {/* 圖案小卡 */}
              <div className={`p-5 rounded-2xl border ${isDarkMode ? 'bg-[#11131E]/90 border-gray-800' : 'bg-white border-gray-200'}`}>
              ...
              <div className={`p-5 rounded-2xl border ${isDarkMode ? 'bg-[#11131E]/90 border-gray-800' : 'bg-white border-gray-200'}`}>
                ...
                <div className="space-y-2.5 max-h-[160px] overflow-y-auto pr-1">
                  {ZENTANGLE_PATTERNS.map(pattern => (
                    <div key={pattern.name} className="p-2 rounded bg-gray-500/5 text-xs flex items-start gap-2.5">
                      <span className="text-base">{pattern.symbol}</span>
                      <div>
                        <div className="font-bold flex items-center gap-1">
                          <span>{pattern.name}</span>
                          <span className="text-[9px] px-1 rounded bg-amber-500/10 text-amber-400">{pattern.difficulty}</span>
                        </div>
            ...
            <div className="lg:col-span-3 space-y-6">
              ...
              <div className={`p-5 rounded-2xl border ${isDarkMode ? 'bg-[#11131E]/90 border-gray-800' : 'bg-white border-gray-200'}`}>
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-[10px] text-gray-500 leading-relaxed">
                    使用純淨聲波諧振原理，專為禪修畫作編排的 110Hz A2 梵音，能在作畫時自然引導大腦進入平穩的 𝜶 腦波狀態。
                  </p>
              ...
              {/* 作畫小貼士與科普 */}
              <div className={`p-5 rounded-2xl border ${isDarkMode ? 'bg-[#11131E]/90 border-gray-800' : 'bg-white border-gray-200'} text-xs leading-relaxed space-y-2`}>
                <div className="flex items-center gap-1 text-amber-500 font-bold font-serif mb-1">
                  <Info className="w-3.5 h-3.5" />
                  <span>禪繞唐卡作畫心法</span>
                </div>
                <p className="opacity-85">
                  <strong>一筆一畫原則：</strong> 禪繞畫沒有「錯誤」。如果不小心畫歪了，不要去擦掉，而是顺著歪掉的線條延伸出一個新的對稱裝飾。
                </p>
                <p className="opacity-85">
                  <strong>呼吸配合：</strong> 如果覺得線條發抖，可以嘗試在「持氣（靜止）」或者平緩「吐氣」時拉出長線條，會感到無比順暢。
                </p>
              </div>
